...
def calculate(input_text):
    ...
    for number in numbers:
        print(f"Call {number}")
        winners, boards = round(boards,number)
        if len(boards) == 0:
                break
    print(f"{winners=}")
    if len(winners) > 1:
        raise ValueError("Mulitple winners!")
    answer = score(winners[0]) * number
    return answer

def round(boards,number):
    ...
    for board in boards:
        ...
        for line in board:
            for i in range(line_length):
                if line[i] == number:
                    line[i] = "X"
                    house = check_house(board)
        if house:
            winners.append(board)
        else:
            next_boards.append(board)
    return winners, next_boards
