    ...
    for i in range(len(sfs)):
        for j in range(len(sfs)):
            if i != j:
                ...
                print(f"{i}: {sfs[i]}")
                print(f"{j}: {sfs[j]}")
                this_sum = ["["] + sfs[i] + sfs[j] + ["]"]
                this_sum_reduced = reduce_sf(this_sum)
                mag = magnitude(this_sum_reduced)
                print(f"{i},{j} -> {mag}")
                if mag > answer:
                    answer = mag
                    print(f"New max {answer}")
...
def magnitude(sf):
    ...
    while True:
        if sf[idx] == "[":
            depth += 1
        elif idx == 1 and isinstance(sf[idx], int):
            # First part of number is integer
            left, right = sf[1], sf[2:-1]
            # print(f"{sf}\n->\n{left}\n%\n{right}\n\n")
...
def test_magnitude7():
    ...
    ans7 = 78
    ex7 = parse_sf("[[6,[6,6]],[[7,0],[8,9]]]")
    ex7 = parse_sf("[6,[6,6]]")

    assert magnitude(ex7) == ans7
